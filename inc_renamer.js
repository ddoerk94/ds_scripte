...
// @include         https://de*.die-staemme.de/game.php*
// @include         https://ch*.staemme.ch/game.php*
// @include         https://zz2.beta.tribalwars.net/game.php*
// @include         https://de*.ds.ignames.net/game.php*
...
function labelAttack() {
    var $ = win.$;
    var ir_forum = 'https://forum.die-staemme.de/showthread.php?116511-LZR-Inc-Renamer&p=3551587&viewfull=1#post3551587';
    var ir_version = '3.0';
    ...
    function fnDateFormat(dtDate) {
        ...
        var zeigeMs = (oStorage.irSettings.mitMS) ? '.' + (intMs > 99 ? intMs : '0' + myZeroPad(intMs)) : '';
        return myZeroPad(dtDate.getHours()) + ':' + myZeroPad(dtDate.getMinutes()) + ':' + myZeroPad(dtDate.getSeconds()) + zeigeMs + ' ' + myZeroPad(dtDate.getDate()) + '/' + myZeroPad(dtDate.getMonth() + 1);
    }
    ...
    if (typeof (theFormat) == 'undefined') var theFormat = '{unit} ({coords}) {origin} {player} {incid} {sent}';
    if (typeof (theFormatII) == 'undefined') var theFormatII = theFormat;
    if (typeof (arrUnitNames) == 'undefined') var arrUnitNames = ['Sp\u00e4h', 'LKAV', 'SKAV', 'Axt', 'Schwert', 'Rammbock', 'Kata', '**AG**', 'UNBK'];
    ...
    function fLadeEinstellungen() {
        ...
        if (sa.config.PlayerStart == null || (sa.config.PlayerStart != oGD.player.date_started)) {
            ...
            sa.config = {
                ...
                UnitSpeed: $(xmldocument).find('unit_speed:first').text(),
                WorldSpeed: $(xmldocument).find('speed:first').text(),
                AGmaxDist: $(xmldocument).find('max_dist:first').text(),
                Millisec: $(xmldocument).find('millis_arrival:first').text(),
            }
        }
    ...
    /* Einstieg */
    var $irTable_0 = $('#quickedit-rename').parents('table').eq(0);
    ...
    /* Ankunft */
    oTexts[8].val = typeof ($Ankunft[0].getElementsByTagName('td')[1].v) == 'undefined' ? $Ankunft[0].getElementsByTagName('td')[1].textContent : $Ankunft[0].getElementsByTagName('td')[1].innerText; // Mar 27, 2014  10:42:03
    ...
    /* Distanz */
    $Ankunft_in = $Ankunft_in.html().match(/\d+/ig);
    ...
    var aZeileD = '<tr><td colspan="1"></td><td colspan="1">Distanz:</td><td colspan="3">' + oTexts[4].val + ' Felder</td></tr>';
    var aTogle = '<form id="BuSet"><a href="#" title="" > <img alt="IR" src="graphic/buildings/garage.png"></img></a></form>';
    var aZeileK = "<tr><th>Einheit</th><th>Abschickzeit</th><th>Laufzeit</th><th>Umbennen in</th></tr>";
    $(aTabLZ).append(aZeileD).append(aZeileK);
    $(aTabLZ).find('td:first').append(aTogle);
    /* Links fÃ¼r VP und Inno-Umbenennung ggf. ausblenden */
    if (!oStorage.irSettings.DS_Links) $($irTable_0).find("a:last").parents('tr').eq(0).hide();
    ...
    for (var theIndex in oStorage.irSettings.UnitNames) {
        ...
        var secsDuration = Math.round([9, 10, 11, 18, 22, 30, 30, 35, 0][theIndex] * 60 * dblDistance / oStorage.config.WorldSpeed / oStorage.config.UnitSpeed);
        ...
        if (secsDiff > 0 || secsDuration == 0) {
            ...
            var aStart = secsDiff < 60 && 'gerade eben' ||
                'vor ' + secsDiff < 3600 && Math.floor(secsDiff / 60) + ' min' ||
                'vor ' + myTime(secsDiff);
            var aZeileU = '<tr><td>' + oStorage.irSettings.UnitNames[theIndex] + '</td><td>' + aStart + '</td><td>' + oTexts[3].val + '</td><td></td></tr>';
            $(aTabLZ).append(aZeileU);
            $(aTabLZ).find('td:last').append(button).append(input);
        }
    }
    /* Hotkeys */ //z.B.  win.arrKeys=[49,50,51,52,53,54,55,56,57];
    ...
    function fTabEinstellungen() {
        ...
        $(aZeileOptions).append(aMsTd).append(aLinkTd).append(aTrennerTd);
        ...
        var aZeileUnitNames = '<tr><td><nobr>Namen der</nobr> Einheiten</td><td><input id="irInUnitNames" onfocus="irInputAktiv = false" type="text" size="90" value="' + oStorage.irSettings.UnitNames + '" </td></tr>';
        var aZeileVersion1 = '<tr><td>Variante 1 </td><td><input id="irInVersion1" onfocus="irInputAktiv = this.id" type="text" size="90" value="' + oStorage.irSettings.Form[0] + '" </td></tr>';
        var aZeileVersion2 = '<tr><td>Variante 2</td><td><input id="irInVersion2" onfocus="irInputAktiv = this.id" type="text" size="90" value="' + oStorage.irSettings.Form[1] + '" </td></tr>';

        /* Bereich der Platzhalter */
        var aZeigePh = fPlatzhalter();
        $(aTabText).append(aZeileUnitNames).append(aZeileVersion1).append(aZeileVersion2);
        $(aTabText).find('tr').eq(parseInt(oStorage.irSettings.Radio_Form) + 1).find('td:first').css('font-weight', "bold");

        /* Speichern-Button */
        var aTabBtn = document.createElement("table");
        aTabBtn.id = "irTabBtn";
        var aZeileBtn = '<tr><td></td><td>Abbruch mit F5</td><td width="128"> </td><td>Inc-Renamer Version ' + ir_version + '</td><td><a target="_blank" href=' + ir_forum + '>zum Forum</a></td></tr>';
        ...
        inButton.onclick = function () {
            ...
            /* Ãœbergabe der Werte  */
            var strFehler = '';
            if ($('#irInUnitNames').val().split(",").length == 9) {
                oStorage.irSettings.UnitNames = $('#irInUnitNames').val().split(",");
            } else {
                strFehler += "- es mÃ¼ssen genau 9 durch Komma getrennte Einheiten sein  ";
            }
        ...
        $(aTabBtn).find('td:first').append(inButton);
        var aTabInputSum = document.createElement("table");
        $(aTabInputSum).append(aTabOptions).append(aTabText).append(aZeigePh).append(aTabBtn);
        return aTabInputSum;
        ...
        function fPlatzhalter() {
            ...
            for (var i = 0; i != oTexts.length; ++i) {
                ...
                $(td).append(a);
                $(td).click(function () {
                    insertPh($(this).text().replace(/\&amp;/g, '&'), gid(irInputAktiv));
                });
                if (i <= 6) {
                    $(tr1).append(td);
                } else {
    ...
    aTabSet.width = '570';
    var aZeileSchnell_1 = '<tr id="irSetZ_01"><td>Variante 1 <input type="radio" name="Form" value="0"></td><td>' + oStorage.irSettings.Form[0] + '</td></tr>';
    var aZeileSchnell_2 = '<tr id="irSetZ_02"><td>Variante 2 <input type="radio" name="Form" value="1"></td><td>' + oStorage.irSettings.Form[1] + '</td></tr>';
    $(aTabSet).append(aZeileSchnell_1).append(aZeileSchnell_2);
    /* fertige Tabellen einfÃ¼gen */
    $(aTabLZ).find(':text').attr('size', name.length + 1);
    $irTable_0.after(aTabLZ);

    /* Einstellungen verwalten */
    var clicktexts = {
        v1: 'Inc-Renamer: 1 * Klicken -> Schnellwahl',
        v2: 'Inc-Renamer: noch mal fÃ¼r Einstellungen',
    ...
    $(clickEinst).bind('click', function (ev) {
        switch ($('#BuSet a:first').attr('title')) {
            case clicktexts.v1: // Schnellauswahl
                ...
                $('#irFenster_01').find("input[name='Form']").eq(oStorage.irSettings.Radio_Form).attr("checked", true);
